...
int *res_is_deny_allow = is_deny_allow(tcmd, "chk_deny");
if(res_is_deny_allow == 0){
	printf("-bash: command not allowed\n");
	return 0;
}else if(res_is_deny_allow == 2){
        printf("-bash: deny/allow command is too long\n");
        return 0;
}
...
int is_deny_allow (command, flg_deny_allow)
{
  char *tcmd = command;
  char *flg = flg_deny_allow;
  FILE *fp;
  char *fname;
  char s[500];
  char *split_user;
  char *split_condition;
  char *split_command;
  int *res_is_allow = 1;
  ...
  if(strcmp(flg, "chk_allow") == 0){
    fname = "/etc/sbash.allow";
  }else{
    fname = "/etc/sbash.deny";
  }
  fp = fopen( fname, "r" );
  if( fp != NULL ){
    while( fgets( s, 500, fp ) != NULL ){
      ...
      split_command = strtok(s, ":");
      split_condition = strtok(NULL, ":");
      split_user = strtok(NULL, ":");
      split_user[strlen(split_user) - 1] = '\0';
        if ( strcmp("ALL", split_user) == 0 || strcmp(user_name, split_user) == 0 ){
          if( (strcmp("=", split_condition) == 0 && strcmp(tcmd, split_command) == 0) || (strcmp(".", split_condition) == 0 && strstr(tcmd, split_command) != NULL) || (strcmp("^", split_condition) == 0 && strncmp(tcmd, split_command, strlen(split_command)) == 0) ){
            if(strcmp(flg, "chk_allow") == 0){
              return 0;
            }else{
              res_is_allow = is_deny_allow(tcmd, "chk_allow");
              if( res_is_allow == 0 ){
                break;
              }
            }
...
int logging_command(command)
{
  char *str = NULL;
  char *tcmd = command;
  FILE *fp;
  char *fname = "/var/log/sbash";

  if( strcmp(tcmd, "\n") == 0 || strcmp(tcmd, "\r") == 0 || strcmp(tcmd, "\r\n") == 0){
    return 1;
  }
  ...
  str = (char *)malloc(sizeof(char) *(strlen(tcmd)+50));

  sprintf(str, "%s %02d-%02d-%02d %02d:%02d:%02d\n%s\n", current_user.user_name, pnow->tm_year+1900, pnow->tm_mon + 1, pnow->tm_mday, pnow->tm_hour, pnow->tm_min, pnow->tm_sec, tcmd);
  fputs(str, fp);
  fclose( fp );
